...
def validate_sale_price(obj):
    ...
    if sale_price < 0:
        raise ValidationError(
            f"Цена товара со скидкой ({sale_price}) не может быть отрицательной."
        )

    discount = 100 - ((sale_price / price) * 100)

    if not 10 <= discount <= 90:
        raise ValidationError(
            f"Скидка на товар должна быть в пределах 10-90%. Текущая скидка - {int(discount)}%"
        )
